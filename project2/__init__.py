...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    for i in tickers:
        dataframe[i.lower()] = read_prc_csv(i)[prc_col]
...
def mk_ret_df(prc_df):
    ...
    """
    data = pd.read_csv(cfg.FF_CSV, usecols=['mkt', 'Date'], parse_dates=['Date'], index_col=['Date'])
    prc_df['mkt'] = data['mkt']
    return prc_df
...
def get_ann_ret(ser, start, end):
    ...
    """
    tot_ret = 1
    r = df.loc[start:end][ser]
    for ret in df.loc[start:end][ser]:
        tot_ret = tot_ret * (1 + ret)
    print(tot_ret)
    return (tot_ret) ** (252 / len(r)) - 1
...
Q4_ANSWER = '?'
tickers = cfg.TICKERS
import project2 as prj
prc_df = prj.mk_prc_df(tickers, prc_col='adj_close')
#print(prc_df)
ret_df = prj.mk_ret_df(prc_df)
#print(ret_df)
aret_df = prj.mk_aret_df(ret_df)
#print(aret_df)




ew_rets = prj.get_ew_rets(ret_df, tickers)
print(prj.get_ann_ret(ew_rets, '2010-01-01', '2020-12-31'))

print('Q3')
print(prj.get_ann_ret(ret_df['TSLA'], '2010-01-01', '2020-12-31'))

print('Q4')
print(prj.get_ann_ret(aret_df['TSLA'], '2010-01-01', '2020-12-31'))
